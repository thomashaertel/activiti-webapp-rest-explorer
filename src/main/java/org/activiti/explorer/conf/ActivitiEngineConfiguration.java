...
//	@Bean(name = "ldapConfigurator")
	public LDAPConfigurator ldapConfigurator() {
		LDAPConfigurator ldap = new LDAPConfigurator();

		ldap.setServer(environment.getProperty("ldap.server"));
		ldap.setPort(Integer.parseInt(environment.getProperty("ldap.port")));
		ldap.setSecurityAuthentication(environment.getProperty("ldap.authmethod"));
		ldap.setUser(environment.getProperty("ldap.user"));
		ldap.setPassword(environment.getProperty("ldap.pass"));
		
		ldap.setBaseDn(environment.getProperty("ldap.basedn"));
		ldap.setUserBaseDn(environment.getProperty("ldap.userbasedn"));
		ldap.setGroupBaseDn(environment.getProperty("ldap.groupbasedn"));
...
/*		ldap.setQueryUserByUserId("(&(objectClass=user)(sAMAccountName={0}))");
        ...
        ldap.setCustomConnectionParameters(connectionMap);*/

        ldap.setUserIdAttribute(environment.getProperty("ldap.users.id"));
        ldap.setUserFirstNameAttribute(environment.getProperty("ldap.users.firstname"));
        ldap.setUserLastNameAttribute(environment.getProperty("ldap.users.lastname"));
         
        ldap.setGroupIdAttribute(environment.getProperty("ldap.groups.id"));
        ldap.setGroupNameAttribute(environment.getProperty("ldap.groups.name"));
	...
	public ProcessEngineConfigurationImpl processEngineConfiguration() {
		...
		String createDefaultUser = environment.getProperty("create.default.users");
		if (!"true".equals(createDefaultUser)) {
			processEngineConfiguration.addConfigurator(ldapConfigurator());
		}
		...
		// custom form type
		formTypes.add(new FileUploadFormType());
